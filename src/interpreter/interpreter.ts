...
import miriWasm from "/rubri/bin/miri.opt.1718474653.wasm?url";

import LibsPath from "/rubri/lib/rustlib/x86_64-unknown-linux-gnu/lib/libaddr2line-b8754aeb03c02354.rlib?url";
...
export async function initInterpreter(): Promise<Interpreter> {
    ...
    const miri_response = await fetch(miriWasm);
    const miri = await miri_response.arrayBuffer();
...
class Interpreter {
    ...
    async run(code: string, printLast: boolean = false): Promise<string> {
        ...
        this.fds[5].dir.get_file("main.rs")!.data = encode(
            `fn main() {\n${code}\n}`
        );
        ...
        const inst = await WebAssembly.instantiate(this.miri, {
            ...
            wasi_snapshot_preview1: strace(this.wasi.wasiImport, [
                "fd_prestat_get",
            ]),
        });
            ...
            new Directory([
                [
                    ...
                    new Directory([
                        [
                            "wasm32-wasi",
                            new Directory([["lib", new Directory([])]]),
                        ],
                        [
                            "x86_64-unknown-linux-gnu",
                            new Directory([
                                    ...
                                    new Directory(
                                        await (async function () {
                                            ...
                                            let files = [
                                                ...
                                                const LibsPathFolder =
                                                    LibsPath.split("/");
                                                LibsPathFolder.pop();
                                                const LibsPathFolderStr =
                                                    LibsPathFolder.join("/") + '/';
                                                console.log(
                                                    "libs",
                                                    LibsPathFolderStr
